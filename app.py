# This is the back-end interface for password acquisition
import os
import sys
import subprocess
...
@app.route('/')
def index():
    return render_template('index.html', password=session.get('password', default=''))
...
def verify_assertion():
    ...
    # password = 'password'
    # server_cmd = 'python vpn/server.py -s 127.0.0.1:8489 -p password -e AES-GCM'
    # client_cmd = 'python vpn/client.py -c 127.0.0.1:8488 -s 127.0.0.1:8489 -p password -e AES-GCM'
    # server_process = subprocess.Popen(server_cmd, stdout=subprocess.PIPE, universal_newlines=True)
    # for stdout_line in iter(server_process.stdout.readline, ""):
    #     yield stdout_line
    # server_process.stdout.close()
    # return_code = server_process.wait()
    # if return_code:
    #     raise subprocess.CalledProcessError(return_code, server_cmd)
    # for line in server_process.stdout:
    #     print('[Server]:', line.decode(), end='')
    # client_process = subprocess.Popen(client_cmd)
    # client_process.wait()
    # for line in client_process.stdout:
    #     print('[Client]:', line.decode(), end='')
    # os.system('python vpn/server.py -s 127.0.0.1:8489 -p password -e AES-GCM')
    # os.system('python vpn/client.py -c 127.0.0.1:8488 -s 127.0.0.1:8489 -p password -e AES-GCM')
    return jsonify({
        'success':
